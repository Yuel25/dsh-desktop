...
- 自定义无边框标题栏、系统托盘和单实例运行（标题栏显示当前 profile）
...
- 内置设置窗口：外观、开机自启（可隐藏到托盘）、DSH 端口、profile 管理
- 内置日志查看器与「复制诊断信息」（应用版本、dsh 版本、当前 profile 等）
- 启动时自动检查新版本，可从托盘一键打开发布页
...
「在新窗口打开 profile」会为所选 profile 启动一个独立的 DSH 实例（自动分配空闲端口）并打开独立窗口；关闭该窗口即结束对应的 DSH 进程。每个实例的日志单独记录为 `dsh.<profile>.stdout.log` / `dsh.<profile>.stderr.log`。
...
- **日志**：内置查看器（dsh 输出、错误、恢复日志及各 profile 窗口日志）；
- **诊断**：一键复制应用版本、dsh 版本、当前 profile 等信息，方便反馈问题；
